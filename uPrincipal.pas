...
type
  ...
  private
    ...
    function buscaValor(pTextFind, pTexto: string): variant;
...
function TFormPrincipal.buscaValor(pTextFind, pTexto: string): variant;
var
...
begin
  try
    busca := '\/' + pTextFind + '\<\/td\>\s*\<td\>\d*\,\d*';
    RegularExpression.Create(busca);
    Match := RegularExpression.Match(pTexto);
    valorTotal := 0;
    if Match.Success then
      valorTotal := Match.Value.Split(['<td>'])[1] + buscaValor(pTextFind, pTexto.Substring(Match.Index));
  except
    valorTotal := 0;
  end;
...
begin
  try
    ...
    queryLocal.SQL.Add
      (retornaConsultaSql
      ('ID,DESCRICAO,DIVID_EX_ATUAL,DIVID_EX_ANTERIOR,COTACAO_ATUAL,DIVID_EX_ANTERIOR_PRC', pPapel));
...
procedure TFormPrincipal.cargaPeterLynch(const pPapel: string = '');
var
  queryLocal: TFDQuery;
  htmlRetorno: TStrings;
  vpa, lpa, valorIntriseco, margemSeguranca: Real;
...
begin
  ...
  try
    ...
    while not queryLocal.eof do
    begin
      labelLocal.Text := format(url_fundamentus, [queryLocal.FieldByName('DESCRICAO').AsString]);

      htmlRetorno.Clear;
      htmlRetorno := TFindInHtml.downloadHTML(queryLocal.FieldByName('DESCRICAO').AsString, url_fundamentus);

      if htmlRetorno.Text.Contains('Cres. Rec (5a)') then
      begin
...
begin
  ...
  begin
    ...
    pvFiltroFinal := pvFiltroFinal + format(' and (TAG_ALONG >= %s )',
      [makeEditValidFormatValue(EditTagAlong, edtfmtReal)]);
